 ...
 * Program : simple kafka producer to keywords to kafka(synchronous approach)
 * This producer program has been created to test the consumers which implements safe exit.
 * A list of topics maintained to send the messages to kafka one by one.
 * */
...
public class producer_keyword_sync_approach {
    ...
    public static void main(String args[]) {
        ...
        //List<String> topicNames = List.of("kafka-topic-one-partition"); // send message to one topic
        ...
        List<String> topicNames = Arrays.asList("kafka-topic-one-partition", "kafka-topic-two-partitions", "kafka-topic-three-partitions"); // send message to three topics

        System.out.println("Producer is about to send the keywords....");
        long currentLineNumber = 0;
        try (BufferedReader br = new BufferedReader(new FileReader("..//data//keywords.txt"))) {
